...
| Defaults to `1024`. Increase if the server handles high throughput and packets are being dropped due to queue saturation.

| `TcpServerSettings`
| Low-level TCP socket configuration
