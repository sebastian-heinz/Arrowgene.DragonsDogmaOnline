...
namespace Arrowgene.Ddon.Database.Sql.Core
{
        ...
        where TReader : DbDataReader
    {
        ...
        public Item SelectItem(uint characterCommonId, string uid)
        {
            Item item = null;
            ...
            reader => 
            {
                if(reader.Read())
                {
                    ...
                    ExecuteReader(SqlSelectAllCrestData,
                        command2 => {
                            AddParameter(command2, "@character_common_id", characterCommonId);
                            AddParameter(command2, "@item_uid", uid);
                        }, reader2 => {
                            while (reader2.Read())
                            {
