...
namespace Arrowgene.Ddon.Database.Sql.Core
{
    ...
    {
        private const string SqlInsertToken = "INSERT INTO `ddo_game_token` (`account_id`, `character_id`, `token`, `created`) VALUES (@account_id, @character_id, @token, @created);";
        private const string SqlUpdateToken = "UPDATE `ddo_game_token` SET `character_id`=@character_id, `token`=@token, `created`=@created WHERE `account_id` = @account_id;";
        private const string SqlSelectTokenByAccountId = "SELECT `account_id`, `character_id`, `token`, `created` FROM `ddo_game_token` WHERE `account_id` = @account_id;";
        private const string SqlDeleteTokenByAccountId = "DELETE FROM `ddo_game_token` WHERE `account_id`=@account_id;";
        private const string SqlSelectToken = "SELECT `account_id`, `character_id`, `token`, `created` FROM `ddo_game_token` WHERE `token` = @token;";
        private const string SqlDeleteToken = "DELETE FROM `ddo_game_token` WHERE `token`=@token;";
