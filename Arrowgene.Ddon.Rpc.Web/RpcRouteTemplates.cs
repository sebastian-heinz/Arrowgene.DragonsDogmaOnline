...
namespace Arrowgene.Ddon.Rpc.Web
{
    ...
    public abstract class RpcPostCommand<T> : RpcCommandReturnable
    {
        protected readonly T _entry;

        protected RpcPostCommand(T entry)
        {
            _entry = entry;
        }
    }

    public abstract class RpcGetCommand : RpcCommandReturnable
    {
        protected readonly WebCollection<string, string> _queryParams;

        protected RpcGetCommand(WebCollection<string, string> queryParams)
        {
            _queryParams = queryParams;
        }
    }

    public abstract class RpcPostRoute<RequestT, CommandT> : RpcWebRoute
        where CommandT : RpcPostCommand<RequestT>
    {
        protected RpcPostRoute(IRpcExecuter executer) : base(executer)
        {
        }
    ...
    public abstract class RpcGetRoute<CommandT> : RpcWebRoute, IWebRoute
        where CommandT : RpcGetCommand
    {
        protected RpcGetRoute(IRpcExecuter executer) : base(executer)
        {
        }
