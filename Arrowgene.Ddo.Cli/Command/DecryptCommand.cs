...
namespace Arrowgene.Ddo.Cli.Command
{
    public class DecryptCommand : ICommand
    {
        ...
        {
            0x71, 0x2D, 0x4B, 0x55, 0x57, 0x59, 0x68, 0x6E, 0x36, 0x62, 0x4B, 0x6E, 0x75, 0x64, 0x45, 0x66, 0x4F, 0x36,
            0x6B, 0x57, 0x45, 0x63, 0x48, 0x49, 0x34, 0x4C, 0x48, 0x6E, 0x6E, 0x4D, 0x70, 0x5F, 0x75, 0x61, 0x66, 0x51,
            0x6F, 0x6A, 0x78, 0x2D, 0x41, 0x54, 0x5F, 0x67, 0x63, 0x54, 0x67, 0x57
        };
        ...
        private byte[] Decrypt(byte[] input)
        {
            ...
            DumpBuffer(new StreamBuffer(test));

            byte[] key = GenerateKey(test);
            Console.WriteLine("Generated Lookup:");
            DumpBuffer(new StreamBuffer(key));
            ...
            byte[] transformed1 = new byte[transformed.Length];
            for (int i = 0; i < transformed.Length; i++)
            {
                uint current = transformed[i];
                uint j = (uint) (i % 4);
                uint j1 = j << 3; // 0369164A | C1E2 03 | shl edx,3 
                byte j2 = (byte) j1;
                uint j3 = (uint) (0xFF << j2);
            ...
            Console.WriteLine("Transformed:");
            DumpBuffer(new StreamBuffer(transformed1));
            Console.WriteLine();


            // Transform transformed
            byte[] destination_addr = new byte[transformed1.Length];
            uint esi;
            uint edx = 0;
            for (int i = 0; i < transformed1.Length; i += 4)
            {
                ...
                uint src = (uint) (transformed1[i] | transformed1[i + 1] << 8 |
                                   transformed1[i + 2] << 16 | transformed1[i + 3] << 24);
                esi = esi - src;
                esi = esi - eax;
                if (edi == esi)
                {
            ...
            Console.WriteLine("Transformed:");
            DumpBuffer(new StreamBuffer(destination_addr));
            Console.WriteLine();
        ...
        private byte[] GenerateKey(byte[] input)
        {
            ...
            GenerateRowB(80, output, 0x0D, 0x13, 4);
            // copy last row to result - reversed
            for (int i = 0; i < 4; i++)
            {
                int offsetSrc = i * 4 + 128;
            ...
            GenerateRowB(112, output, 0x1C, 0x04, 4);
            // copy last row to result - reversed
            for (int i = 0; i < 4; i++)
            {
                int offsetSrc = i * 4 + 128;
            ...
            GenerateRowB(64, output, 0x0D, 0x13, 8);
            // copy last row to result - reversed
            for (int i = 0; i < 4; i++)
            {
                int offsetSrc = i * 4 + 128;
                int offsetDst = i * 4 + 144;
                result[offsetDst + 0] = output[offsetSrc + 3];
                result[offsetDst + 1] = output[offsetSrc + 2];
                result[offsetDst + 2] = output[offsetSrc + 1];
                result[offsetDst + 3] = output[offsetSrc + 0];
            ...
            GenerateRowB(80, output, 0x0D, 0x13, 8);
            // copy last row to result - reversed
            for (int i = 0; i < 4; i++)
            {
                int offsetSrc = i * 4 + 128;
                int offsetDst = i * 4 + 160;
                result[offsetDst + 0] = output[offsetSrc + 3];
                result[offsetDst + 1] = output[offsetSrc + 2];
                result[offsetDst + 2] = output[offsetSrc + 1];
                result[offsetDst + 3] = output[offsetSrc + 0];
            ...
            GenerateRowB(96, output, 0x1E, 0x02, 8);
            // copy last row to result - reversed
            for (int i = 0; i < 4; i++)
            {
                int offsetSrc = i * 4 + 128;
                int offsetDst = i * 4 + 176;
                result[offsetDst + 0] = output[offsetSrc + 3];
                result[offsetDst + 1] = output[offsetSrc + 2];
                result[offsetDst + 2] = output[offsetSrc + 1];
                result[offsetDst + 3] = output[offsetSrc + 0];
            ...
            GenerateRowB(80, output, 0x1E, 0x02, 8);
            // copy last row to result - reversed
            for (int i = 0; i < 4; i++)
            {
                int offsetSrc = i * 4 + 128;
                int offsetDst = i * 4 + 192;
                result[offsetDst + 0] = output[offsetSrc + 3];
                result[offsetDst + 1] = output[offsetSrc + 2];
                result[offsetDst + 2] = output[offsetSrc + 1];
                result[offsetDst + 3] = output[offsetSrc + 0];
            ...
            GenerateRowB(64, output, 0x0F, 0x11, 12);
            // copy last row to result - reversed
            for (int i = 0; i < 4; i++)
            {
                int offsetSrc = i * 4 + 128;
                int offsetDst = i * 4 + 208;
                result[offsetDst + 0] = output[offsetSrc + 3];
                result[offsetDst + 1] = output[offsetSrc + 2];
                result[offsetDst + 2] = output[offsetSrc + 1];
                result[offsetDst + 3] = output[offsetSrc + 0];
            ...
            GenerateRowB(112, output, 0x0F, 0x11, 12);
            // copy last row to result - reversed
            for (int i = 0; i < 4; i++)
            {
                int offsetSrc = i * 4 + 128;
                int offsetDst = i * 4 + 224;
                result[offsetDst + 0] = output[offsetSrc + 3];
                result[offsetDst + 1] = output[offsetSrc + 2];
                result[offsetDst + 2] = output[offsetSrc + 1];
                result[offsetDst + 3] = output[offsetSrc + 0];
        ...
        private void GenerateRowB(int outputIndex, byte[] output, byte shl, byte shr, int startOffset = 0)
        {
            ...
            while (read < 16)
            {
                uint b = (uint) (output[current] | output[current + 1] << 8 |
                                 output[current + 2] << 16 | output[current + 3] << 24);
                current += 4;
                ...
                uint a = (uint) (output[current] | output[current + 1] << 8 |
                                 output[current + 2] << 16 | output[current + 3] << 24);
        ...
        private static byte[] RootCryptoKey1 = new byte[]
        {
            0x66, 0x32, 0x33, 0x65, 0x39, 0x38, 0x48, 0x61, 0x66, 0x4A, 0x64, 0x53, 0x6F, 0x61, 0x6A, 0x38,
            0x30, 0x51, 0x42, 0x6A, 0x68, 0x68, 0x32, 0x33, 0x6F, 0x61, 0x6A, 0x67, 0x6B, 0x6C, 0x53, 0x61,
        };
